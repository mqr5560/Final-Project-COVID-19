...
Lines 21, 22, and 23 I load in different data sources. 

(B) .Rmd Line numbers for inspecting data intake:  
Line 63, I look at the data table output, as well as line 85 and other random spots throughout the project. Assumed you only need a few and not all of them. 
...
Line 106 I am deleting unnecessary columns from my data. Row 117 I am renaming the columns and summarising cases. 

(B) .Rmd Line number(s) for a join operation: 
NA. 

(C) .Rmd Line number(s) for a spread or gather operation (or equivalent):
Line 124 I use the t() function to transpose the data and switch the variables and cases. 

(D) .Rmd Line number(s) for use of regular expressions: 
Line 206 I use gsub to get rid of all the . in the variable names so I could then classify them as dates further on. 

(E) .Rmd Line number(s) for use of reduction and/or transformation functions: 
Line 177 I use the group_by() function as well as in Line 79 and various places throughout my code. I also use the summarise function frequently, to name a few on Line 80, 58, and 35. 

(F) .Rmd Line number(s) for use of user-defined functions: 
Line 226 I defined the function to change to a specific date for all my variables as m, d, and y. Didn't use the lubridate function here and did my own. 
...
Line 46 I use geom_bar but in line 242 I used geom_smooth. 

(B) .Rmd Line number(s) for use of multiple aesthetics:  
Line 90 used aes to define the graph variables and then in line 91 used aesthetics for the color of the bars as well as their fill color. Line 304, used aesthetic to color the vertical bars to look at the different COVID-19 responses of the countries. 
(C) .Rmd Line number(s) for use of layered graphics:  
Line 303 included title, and also in 302 included a smoother for the data. Line 304 and 305 I incldued vertical lines, that went over the data to show when COVID-19 protocols were implemented in Germany. 
